		...
		init: {
			FilesAdded: function(up, files) {
				plupload.each(files, function(file) {
					...
					$('#'+th).find('.uploadfilename').html(file.name + ' (' + plupload.formatSize(file.size)+')' );
					if (filename != '') {
			...
			UploadProgress: function(up, file) {
				$('#'+th).find('.uploadspan').html(file.percent + "%");
			},

			FileUploaded: function(up, file, info) {
	            if (info.status == 200){
	            	var imgurl = '{{$ossconfig["host"].'/'.$ossconfig["dir"]}}'+hex_md5(file.name)+get_suffix(file.name) ;
	                $('#'+th).find('.uploadimg').attr('src',imgurl);
	                $('#'+th).find('.uploadinput').val(imgurl);
	                $('#'+th).find('.filenameshow').attr('href',imgurl).show();
	            }else{
	            	alert('上传失败');
	            }
			},
...
$('.selectbtn ').on('click',function(){
	th = $(this).parents('.selectbtnbody').attr('id');
})
